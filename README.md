...
| glz-util    | 個人的なライブラリで汎用的な用途に耐えられないため                         |
| jpegli      | 公式の方針でlibjpeg-turboと一部ファイルが重複するportは登録できないため    |
| uuid7pp     | 個人的なライブラリで汎用的な用途に耐えられないため                         |
| yase-json   | 個人的なライブラリで汎用的な用途に耐えられないため                         |
| snappy      | `<immintrin.h>` を明示的にインクルードするバグ修正を含むため               |
| wxwidgets   | 公式の方針でUTF8をサポートするためのコンパイルオプションを追加できないため |
| zlib-ng     | 公式の方針でネイティブCPU最適化を有効にできないため                        |
| zxc         | 公式の方針でネイティブCPU最適化を有効にできないため                        |
